...
def register_affine(matched_points, fixed_indices):
    ...
    for idx, midx in idx_to_model_idx.items():
        res[idx] = r[mat_len*midx:mat_len*midx+mat_len].reshape((ndim, ndim+1))
        
    for idx in fixed_indices:
        res[idx] = np.eye(ndim, ndim+1)
...
def register_translations(matched_points, fixed_indices):
    ...
    res = {}
    for idx, midx in idx_to_model_idx.items():
        res[idx] = np.hstack((np.eye(ndim), r[mat_len*midx:mat_len*midx+mat_len].reshape((ndim, 1))))
...
def aug_mat(arr):
    """
    Augment an $ndim/times(ndim+1)$ affine+translation matrix with an additional 0, 0, ..., 1 row
